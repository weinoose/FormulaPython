def upgrade(engineer_crew,designer,cto,aerodynamicst,min_budget,max_budget,spent_budget):
    ...
    return round(round(((((round(((phase_1) + (phase_2) + (phase_3)),3))*1.417)**1.3)/1.3),3))
...
def design(designer_name,designer,cto,aerodynamicst,focus,focus_r,min_budget,max_budget,spent_budget,engineer_crew,pit_package,FW_R,RW_R,chassis_R,base_R,sidepod_R,suspension_R): 
    ...
    return f"Manufacturer($,{CREW},$,{round(CHASSIS)},{round(FW)},{round(RW)},{round(BASE)},{round(SIDEPOD)},{round(SUSPENSION)},{round(RELIABILITY)},{WEIGHT},'{focus}')"

def driver(automated,status,quality,style,char,preferences,w):
    if status != 'Rookie':
        P1 = 14
    else:
        P1 = 4
    ...
    if automated == True:
        ...
        w = choice(['Balanced','Balanced','Balanced','Balanced','Balanced','Balanced','Balanced'
    ...
    return f"Driver('$','$',None,None,{pace},{braking},{smoothness},{adaptability},{consistency},{fitness},{aggression},{attack},{defence},{start},{wet},[None],'{w}')"