...
class Tire():
    ...
    def laptime(self,driver,circuit,lap,tire_usage,mode):
        ...
        if driver.style != driver.team.style:
            ...
            if mode[0] == 'saturday':
                CAR_DRIVER_CHEMISTRY = uniform(0.000,0.050)*(-1.0)
            else:
                CAR_DRIVER_CHEMISTRY = uniform(0.000,0.075)*(-1.0)

        # # # 3.1: Best Track, Best Lap
        if circuit.location in driver.favorite:
            if mode[0] == 'sunday':
                BEST = uniform(0.000,0.125)
            else:
                BEST = uniform(0.000,0.075)
        else:
            BEST = 0
        ...
        if mode[0] == 'sunday': 
            if lap == 1:
                return (CL0) + (CL1/3) + (CL2/3) + (GRID_EFFECT) - (circuit.laptime/85)
            else:
...
redbull = Manufacturer('Oracle Red Bull Racing',RB,HONDA,89,89,95,95,91,91,+0.00,'Balanced',0.22)
ferrari = Manufacturer('Scuderia Ferrari',SF,FERRARI,89,89,89,86,89,89,+0.00,'Stiff Front',0.14)
astonmartin = Manufacturer('Aston Martin Aramco Cognizant F1 Team',AMR,MERCEDES,84,88,82,94,89,89,+0.00,'Stiff Front',0.19)
mercedes = Manufacturer('Mercedes-AMG Petronas F1 Team',MER,MERCEDES,86,86,86,91,91,91,+0.00,'Unbalanced',0.18)
alpine = Manufacturer('BWT Alpine F1 Team',ALP,RENAULT,84,82,82,84,84,84,+0.00,'Unbalanced',0.16)
haas = Manufacturer('Haas F1 Team',HAAS,FERRARI,82,82,82,81,81,81,+0.00,'Balanced',0.17)
alfaromeo = Manufacturer('Alfa Romeo F1 Team Orlen',ALFA,FERRARI,79,79,79,79,79,79,+0.00,'Unbalanced',0.12)
mclaren = Manufacturer('McLaren F1 Team',MCL,MERCEDES,76,76,76,81,81,81,+0.00,'Balanced',0.15)
alphatauri = Manufacturer('Scuderia AlphaTauri',AT,HONDA,77,77,76,76,76,76,+0.00,'Balanced',0.14)
williams = Manufacturer('Williams Racing',WIL,MERCEDES,77,77,77,77,81,81,+0.00,'Balanced',0.13)
manufacturers = [redbull,ferrari,astonmartin,mercedes,alpine,haas,alfaromeo,mclaren,alphatauri,williams]
...
ALO = Driver(astonmartin,'Fernando Alonso','ESP',14,87,89,92,91,90,93,86,93,94,94,91,['Budapest','Silverstone','Monza','Barcelona','Valencia'],'Stiff Front') # 90.272 > 90
...
GAS = Driver(alpine,'Pierre Gasly','FRA',10,88,87,85,86,85,85,81,84,81,87,85,['Monza'],None) # 85.414 > 85
...
PIA = Driver(mclaren,'Oscar Piastri','AUS',None,87,82,82,81,82,86,80,80,80,81,82,[None],None) # 82.488 > 83
# MSC = Driver(None,'Mick Schumacher','GER',47,80,80,82,82,79,81,85,88,84,83,83,['Spielberg'],'Balanced') # 81.676 > 82
DEV = Driver(alphatauri,'Nyck de Vries','NET',None,81,81,83,85,79,82,82,82,82,83,83,[None],None) # 81.638 > 82
...
HUL = Driver(haas,'Nico Hulkenberg','GER',None,84,84,80,80,77,81,79,79,79,82,77,[None],None) # 80.116 > # 80
...
SAR = Driver(williams,'Logan Sargeant','USA',None,79,74,80,80,80,80,80,81,80,80,70,[None],None) # 77.481 > # 77
...
drivers = [VER,LEC,HAM,ALO,PER,NOR,RUS,SAI,BOT,OCO,STR,GAS,MAG,ALB,TSU,PIA,DEV,HUL,ZHO,SAR]
